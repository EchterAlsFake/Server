---
title: "Supported platforms — Thumbzilla API"
summary: "Lists the operating systems and Python versions documented for the Thumbzilla API."
public_url: "https://docs.echteralsfake.me/thumbzilla/"
aliases:
  - "Thumbzilla Supported platforms"
keywords:
  - "Thumbzilla"
  - "Supported platforms"
---

# Supported platforms — Thumbzilla API

Lists the operating systems and Python versions documented for the Thumbzilla API.

Platform| Architecture| Status
---|---|---
Windows 11| x64| ✅ Tested
macOS Sequoia| x86_64 / arm64| ✅ Tested
Linux (Arch)| x86_64| ✅ Tested
Android 16| aarch64| ✅ Tested

## Related MCP documents

- [Thumbzilla API getting started](getting-started.md)
- [Errors and troubleshooting — Thumbzilla API](troubleshooting/errors.md)
- [Overview — eaf_base_api](../eaf-base-api/overview.md)
- [Legal disclaimer for EAF API wrappers](../legal/disclaimer.md)

## Original public page

- [https://docs.echteralsfake.me/thumbzilla/](https://docs.echteralsfake.me/thumbzilla/)
