---
title: "Pagination, iteration, and ScrapeResult — XVideos API"
summary: "Documents Pagination, iteration, and ScrapeResult behavior, signatures, fields, constraints, and examples for the XVideos API."
public_url: "https://docs.echteralsfake.me/xvideos/"
aliases:
  - "XVideos Pagination, iteration, and ScrapeResult"
keywords:
  - "XVideos"
...
# Pagination, iteration, and ScrapeResult — XVideos API

Documents Pagination, iteration, and ScrapeResult behavior, signatures, fields, constraints, and examples for the XVideos API.

Methods like `search()`, `get_playlist()`, and `channel.videos()` return async generators that yield typed `ScrapeResult[Video]` values. Use `succeeded` to branch safely or `unwrap()` to return the item and raise its terminal error on failure:

```python
async for result in client.search("query", pages=3):
    if result.succeeded:
        video = result.unwrap()
        print(video.title)
    else:
...
Type: Video | None; Description: The populated video on success
...
async for result in client.search(
    "query", pages=3, iterator_config=iterator_config
):
    if result.succeeded:
...
Authenticated account iterators also accept `iterator_config`; their required `POST` request is selected internally, so do not configure private transport fields.
