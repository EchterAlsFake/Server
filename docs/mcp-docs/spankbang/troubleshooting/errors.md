...
`VideoIsProcessing` derives from `ScraperException` and now has a non-empty default exception message. HLS and RAW download failures both retain their causes; a `ResourceGone` raised during downloading is translated to `VideoUnavailable` with the video URL.
