"""Synthetic Ed25519 fixtures, never production keys."""
import base64
    ...
    data = {"meta": {"issued": iso(now), "expiry": iso(now+ttl), "ttl": ttl},
            "data": {"type": "machines", "id": MACHINE, "attributes": {"fingerprint": fingerprint},
                     "relationships": {name: {"data": {"id": value}} for name,value in
                                       (("account", ACCOUNT), ("product", PRODUCT), ("license", LICENSE))}},
            "included": [license_data]}
    data.update(changes)
    encoded = base64.b64encode(json.dumps(data).encode()).decode()
    doc = {"enc": encoded, "sig": base64.b64encode(PRIVATE.sign(("machine/"+encoded).encode())).decode(),
           "alg": "base64+ed25519"}
    return "-----BEGIN MACHINE FILE-----\n" + base64.b64encode(json.dumps(doc).encode()).decode() + "\n-----END MACHINE FILE-----\n"
